...
public class CMUtils {
	 ...
	 *
	 * @param hostname the hostname where Cloudera Manager is running
	 * @param username the username
	 * @param password the password
	 * @return the config files base directory
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	public static File getConfigFiles(String hostname, String username,
       ...
       if(root.getCurrentVersion().compareTo("v3")<0){
    	   throw new RuntimeException("Cloudera Manager must support version 3 of REST API or higher and is currently at " + root.getCurrentVersion());
       }
		...
		for (ApiCluster cluster : clustersResource.readClusters(DataView.FULL)) {
			...
			for (ApiService service : servicesResource
				...
				if (service.getType().equals("MAPREDUCE")) {
					...
					return Utils.unzipConfig(getConfigFiles(
							servicesResource, service.getName()));
